...
def chars_to_bin(chars):
    ...
    return ''.join([bin(ord(c))[2:].zfill(8) for c in chars])#зачем тут [2:]
...
def decode_loop(encoded):
    ...
    for encoded_chunk in chunk_iterator(encoded, CHUNK_LENGTH + 3):  #выделяем чанки из encoded 
        ...
        bin_result_unfixed += ''.join(code_copy[0:len(code_copy)-len(syndrom)])
        ...
        bin_result_fixed += ''.join(code_copy[0:len(code_copy)-len(syndrom)])

        #fixed_encoded_list.append(code_copy[0:len(code_copy)-len(syndrom)])#в конечный list добавляем данные без циклического кода
        fixed_encoded_list.append(''.join(code_copy[0:len(code_copy)-len(syndrom)]))#в конечный list добавляем данные(str) без циклического кода

        for fixed_chunk in fixed_encoded_list:
            for clean_char in [fixed_chunk[i:i + 8] for i in range(len(fixed_chunk)) if not i % 8]:#зачем тут if not i%8
