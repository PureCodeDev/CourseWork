...
def chat(ser,ser2):
    ...
    def fn_send2(message="",redirect=False):
        global counter2
        # global user_name
        if redirect:
            ser2.ft_write(message)
            return
        out_st2 = enter.get()
        if len(out_st2) > 0:
            ser2.ft_write(ser.username + "|" + out_st2 + '\r\n')
            listbox2.insert(tk.END,
                           "[" + datetime.strftime(datetime.now(), "%H:%M:%S") + "] " + ser2.username + ": " + out_st2)
        ...
        while len(in_list2) > 0:
            ...
            if ser2.another_username != None:
                listbox2.insert(tk.END, "[" + datetime.strftime(datetime.now(),
                                                            "%H:%M:%S") + "] " + ser2.another_username + ": " + st2)
                listbox2.itemconfig(counter2, {'fg': 'red'})
                counter2 += 1
                buffer_for_dest_message2.append(
                    "[" + datetime.strftime(datetime.now(), "%H:%M:%S") + "] " + ser2.another_username + ": " + st2)
            else:
                listbox2.insert(tk.END, "[" + datetime.strftime(datetime.now(), "%H:%M:%S") + "] " + ">>> " + st2)
                listbox2.itemconfig(counter2, {'fg': 'red'})
                counter2 += 1
                buffer_for_dest_message2.append("[" + datetime.strftime(datetime.now(), "%H:%M:%S") + "] " + ">>> " + st2)
            # try:
            #     listbox_dest2.insert(tk.END, st2)
            # except:
            #     print("Destination message window is closed")
        if out_flag2:
            fn_send2()
    ...
    def source_message2(): 
        ...
        if counter_source_window2 == 0:
            window_source_message2 = tk.Toplevel(window) 

            def close_window():
                global counter_source_window2
                counter_source_window2 -= 1
                window_source_message2.destroy()
                button_source_message2.config(state='normal')

            window_source_message2.protocol("WM_DELETE_WINDOW", close_window)
            window_source_message2.title('Исходящие сообщения')

            x_cordinate = int((screen_width/2) - 600)
            y_cordinate = int((screen_height/2) - (400/2))

            window_source_message2.geometry("600x400+{}+{}".format(x_cordinate, y_cordinate+50))

            #window_source_message.geometry('600x400+500+200')
            window_source_message2.configure(bg='white')
            listbox_source2 = tk.Listbox(window_source_message2, font=('Arial', 12), bg='white')
    ...
    def dest_message2():
        ...
        if count_dest_window2 == 0:
            window_dest_message2 = tk.Toplevel(window)

            def close_window2():
                global count_dest_window2
                count_dest_window2 -= 1
                window_dest_message2.destroy()
                button_dest_message2.config(state='normal')

            window_dest_message2.protocol("WM_DELETE_WINDOW", close_window2)
            window_dest_message2.title('Входящие сообщения')

            x_cordinate = int((screen_width/2))
            y_cordinate = int((screen_height/2) - (400/2))

            window_dest_message2.geometry("600x400+{}+{}".format(x_cordinate, y_cordinate+50))

            #window_dest_message.geometry('600x400+800+200')
            window_dest_message2.configure(bg='white')
            listbox_dest2 = tk.Listbox(window_dest_message2, font=('Arial', 12), bg='white')
