...
def chat(ser,ser2):
                    ...
                    if ser.is_open:
                        ...
                        if in_st == "ACK_LINKACTIVE":
                            ...
                            if in_st != '':
                                dest_username = ''
                                i = 0
                                while (in_st[i] != '|' and in_st[:3]!= "ACK"):
                                    dest_username += in_st[i]
                                    i += 1
                                if (dest_username != ser.username):
                                    fn_send(in_st)
                                    in_list.append("Redirecting: "+ in_st + " ...")
                                else:
                                    in_list.append(in_st[i+1:])
    ...
    def fn_send(out_st = ""):
        ...
        dest_name = 'C'
        if len(out_st) > 0:
            ser2.ft_write((dest_name + '|' + out_st + '\r\n'))
            listbox.insert(tk.END,
